...
| **TurnResolver** | `choose(choice)` async. Emits `choice_selected` → awaits `onChoiceSelected` → resolves via compiled `ActionStep[]` (no runtime parse) → commits state → awaits `onTurnResolved` → emits `turn_resolved` → emits `state_changed`/`fragment_changed` **only if changed**. Also supports `activateHotspot`. | `TurnResolver.resolve(choice, state, store)` synchronous. Duplicates state twice (previous + current), parses `choice.action` string at runtime via `ActionResolver.resolve`, resolves next fragment, applies entry effects, commits, emits `turn_resolved` signal (previous, current, choice, fragment). `ChronicaSession.choose` then emits `choice_selected` / `fragment_changed` / `state_changed` **unconditionally**. No hotspot resolver. | Partial | Medium | (a) Order of `choice_selected` vs modules differs (main: modules first, mobile: event first). (b) Mobile emits change events conditionally, main unconditionally. (c) Mobile drops `previousState` from `TurnResult`. Fix low-risk by adding `previousState` to `TurnResult` and matching main's unconditional emit for cross-engine listeners. |
...
| **Event order** | choose: `choice_selected` → `onChoiceSelected` → resolve → `onTurnResolved` → `turn_resolved` → `state_changed` (if changed) → `fragment_changed` (if changed). | choose: `TurnResolver` runs modules first (via `turn_resolved` signal → `_on_turn_resolved` → module hooks) → `choice_selected` → `fragment_changed` → `state_changed`, **unconditionally**. | Partial | Medium | Cross-engine listeners that assume main's order will misfire on mobile. See R2. |
| **choice_selected payload** | `{ choice }`. | `{ choice, fragment }` (includes the resulting fragment). | Close | Low | Add `fragment` to mobile's `choice_selected` payload for parity. Backwards-compatible additive field. See R2. |
| **session_started payload** | `{ fragment }`. | `{ location, fragment }`. | Close | Low | Add `location` to mobile's payload additively. See R2. |
| **Module lifecycle hooks** | `initialize`, `onSessionStart`, `onChoiceSelected`, `onTurnResolved`, `onSessionSave`, `onSessionLoad`. Sync or async; failures isolated via `module_error`. Deterministic in registration order. | `initialize`, `on_turn_resolved`, `on_session_loaded`, `on_session_save`, `on_session_load`, plus `get_registry_config` / `apply_registry_config`. Sync only. Ordered by `priority` ASC then registration order. **No `onSessionStart`, no `onChoiceSelected`.** | Partial | Medium | Mobile has richer hook set. `priority` is missing on mobile modules (main sorts by module `priority` before registration order). Cross-engine modules will still run — but ordering across two modules with different priorities won't match. Add optional `priority?: number` to `ChronicaModule` and sort accordingly. Preserve registration-order tie-break. See R3. |
...
| **Package format manifest** | `engine/chronica-package.ts` (unchanged by compat) matches main's format manifest closely: `format`, `version`, `app`, `exportedAt`, `title`, `gameId`, `storyContentHash`, `assetsManifest[]`, `assetCount`, `storySchemaVersion`. Mobile supports story schemaVersion 1..2; main supports 1..3. | Same fields. `SCHEMA_VERSION_MAX := 3` (mobile is stuck at 2). `gameId` is regex-validated: `^[A-Za-z0-9_-]{1,128}$`. | Partial | High | (a) A main-engine story with `schemaVersion: 3` is rejected by mobile's compat validator. (b) Mobile does not enforce `gameId` character constraints. Both are additive fixes. See R6. |
...
4. **Mobile compat validator caps story `schemaVersion` at 2**; main already supports 3. A schema-v3 main-engine story is rejected before ingestion runs. Bumping the cap needs a mobile-side migration path.
5. **Module hook contract differs**: mobile fires `onChoiceSelected` (main has no analog) and lacks a `priority` field for hook ordering (main sorts by `priority` before registration). Cross-engine modules will run, but ordering across multi-module packages won't match.
...
**R2. Align `choose` event shape (small).**
- Include `fragment` in `choice_selected` payload.
- Include `location` in `session_started` payload.
- Emit `fragment_changed` / `state_changed` unconditionally on every successful `choose`. Keep the current "if changed" gating as an internal note or drop it — main's behavior is unconditional.
Consider (larger) reordering: modules run before `choice_selected` to match main. Flag as a separate follow-up so it can be reviewed in isolation.

**R3. Add module `priority` field (small).**
Extend `ChronicaModule` with an optional `priority?: number` (default 0). Change `ModuleRegistry.callHook` / `initializeAll` / `saveAll` / `loadAll` to sort by priority ASC then registration order. Zero impact on modules that don't set the field.
...
**R6. Bump the compat validator's `schemaVersion` cap when project-migration.ts covers v3 (medium).**
Blocked on adding a v2→v3 migration path in `engine/project-migration.ts`. Do the migration first, then flip the cap. Optionally add a `gameId` pattern check (warning, not error) matching main's `^[A-Za-z0-9_-]{1,128}$`.
